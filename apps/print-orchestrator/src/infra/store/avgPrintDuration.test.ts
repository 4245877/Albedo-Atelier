...
import type { PrinterLiveStatus } from "../printers/status/types";
...
test("averages the spans of runs it watched start and finish", async () => {
  ...
  assert.equal(poller.getTodayAvgPrintMs(), null, "no completed run yet → нет данных");
  ...
  await poller.pollOnce();
  assert.equal(poller.getTodayAvgPrintMs(), 40 * MIN);
  ...
  script = { status: "idle", stateText: "complete" };
  await poller.pollOnce();
  assert.equal(poller.getTodayAvgPrintMs(), 50 * MIN);

  assert.equal(poller.serializeToday().avgDurationCount, 2);
  assert.equal(poller.serializeToday().avgDurationMsTotal, 100 * MIN);
});
...
test("a pause is included in the timed duration", async () => {
  ...
  assert.equal(poller.getTodayAvgPrintMs(), 60 * MIN);
});
...
test("a cancelled run is not counted", async () => {
  ...
  assert.equal(poller.getTodayAvgPrintMs(), null, "a cancel contributes nothing");
...
test("a run already printing at startup has no known start and is excluded", async () => {
  ...
  assert.equal(poller.getTodayDone(), 1, "it still counts as completed");
  assert.equal(poller.getTodayAvgPrintMs(), null, "but an untimed run is left out of the average");
});
