 ...
 * precise cases; getTodayHoursUsed() covers the hours conversion and the sum
 * across printers.
 */
...
test("accrues observed printing time and sums it across every printer", async () => {
  ...
  assert.equal(poller.serializeToday().printingMs, 0);

  fakeNow += 3 * MIN;
  await poller.pollOnce();
  // Two printers × 3 min each = 6 observed printer-minutes (the same summation
  // that makes "3 printers × 2 h = 6 h").
  assert.equal(poller.serializeToday().printingMs, 6 * MIN);
  assert.equal(poller.getTodayHoursUsed(), 0.1);

  fakeNow += 3 * MIN;
  await poller.pollOnce();
  assert.equal(poller.serializeToday().printingMs, 12 * MIN);
  assert.equal(poller.getTodayHoursUsed(), 0.2);
});
...
test("the first interval after start is not reconstructed, only observed", async () => {
  ...
  fakeNow += 3 * MIN;
  await poller.pollOnce();
  assert.equal(poller.serializeToday().printingMs, 3 * MIN);
});
...
test("an offline stretch and the reconnect are not counted as printing", async () => {
  ...
  assert.equal(poller.serializeToday().printingMs, 9 * MIN);
});
...
test("a huge gap between polls is capped, not credited in full", async () => {
  ...
  const ms = poller.serializeToday().printingMs;
...
test("an interval spanning local midnight credits only the part after midnight", async () => {
  ...
  assert.equal(poller.serializeToday().printingMs, 2 * MIN);
  assert.equal(poller.serializeToday().key, "2026-07-03");
});
