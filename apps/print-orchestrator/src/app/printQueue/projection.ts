...
/**
 * A held entry or a task parked for the operator shows as `review`; everything
 * else eligible to run shows as `ready` — the same two-status contract the
 * dashboard's legacy queue used.
 */
...
export function toLegacyQueueJob(row: QueueProjectionRow): QueueJob {
  ...
  if (task.reason) job.reason = task.reason;
