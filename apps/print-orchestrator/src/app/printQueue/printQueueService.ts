...
import { ExecutableEnqueue } from "./executableEnqueue";
...
export class PrintQueueService {
  ...
  private readonly executables: ExecutableEnqueue;
  ...
  constructor(
    ...
    options: {
      ...
      /** Farm-config resolver; lets promote name the file the TARGET device starts. */
    ...
    this.executables = new ExecutableEnqueue(ctx, this.queries);
  }
  ...
  /**
   * The slicing-free twin of {@link promoteSliceVariant}: puts an uploaded
   * G-code or sliced 3MF into the queue. See {@link ExecutableEnqueue} for why
   * this is its own use case and not a flag on `releaseTask`.
   */
  enqueueExecutableArtifact(
    taskId: string,
    input: { onDeviceFile?: string | null; material?: string | null } = {},
    actor?: string
  ): TaskDetail {
    return this.executables.enqueue(taskId, input, actor);
  }

  /**
   * Re-derives a task's on-device file name for the printer it is about to run
   * on, when the current name was generated by us and the target's container
   * differs (`x.gcode` on Klipper, `x-<sha8>.gcode.3mf` on a Bambu).
   *
   * Needed because an uploaded executable is queued before any printer is
   * chosen, so its name was built from the artifact's own extension. Leaving it
   * would hand a Bambu a plate package under a `.gcode` name. An operator's
   * hand-picked path is never touched — `isGeneratedDeviceFileName` is what tells
   * the two apart.
   */
  retargetDeviceFile(taskId: string, printerId: string, actor?: string): PrintTask {
    return this.tasks.retargetDeviceFile(taskId, printerId, actor);
  }
