...
/* Дедлайн ОДНОГО запроса запуска.
   ...
   POST /launch делает всю цепочку — сборку пакета, загрузку файла на принтер,
   проверку доставки и ожидание подтверждения старта, — и сервер ограничивает
   каждый шаг отдельно. Клиент обязан сдаваться ПОСЛЕ сервера: иначе оператору
   говорят «запуск не удался» в тот момент, когда принтер его ещё начинает, и
   единственный, кто знает правду, оказывается тем, кого оборвали.

   Значение здесь подбиралось вручную и разошлось с сервером: 240 с против
   серверного худшего случая в 336 с (передача 180 с + повторный листинг для
   проверки + подтверждение старта 45 с). Теперь оно выведено из самих шагов —
   apps/print-orchestrator/src/app/launch/budget.ts, LAUNCH_CLIENT_DEADLINE_MS —
   и тест launchBudget.test.ts падает, если эти два числа снова разойдутся. */
const LAUNCH_TIMEOUT_MS = 366000;
