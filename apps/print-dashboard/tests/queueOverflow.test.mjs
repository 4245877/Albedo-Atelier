...
test("Зал рисует не больше предела строк, а хвост называет число и ведёт в планировщик", () => {
  ...
  // Заголовок строки — кнопка (ведёт в карточку задания), поэтому ищем её по
  // действию, а не по точной разметке класса: считаем именно строки очереди.
  const shown = (body.match(/<li class="row[\s\S]*?<\/li>/g) || [])
    .filter((li) => /data-act="task"[\s\S]*?Задание \d+/.test(li));
